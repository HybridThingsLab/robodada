class EmotionDetectionController {
    ...
    async onPlay(videoElement) {
        ...
        if (result) {
            ...
            if ((this._lastEmotion != expressionSort[0][0]) && (expressionSort[0][1] > 0.7)) {
                //if (this._lastEmotion != expressionSort[0][0]) {
                this._lastEmotion = expressionSort[0][0];

                let event = new CustomEvent("notifyEmotionChanged", {
                    detail: this._lastEmotion
                });
                dispatchEvent(event);
            }

            let newTrkResultEvt = new CustomEvent("notifyNewTrackResult", {
                detail: result
            });
